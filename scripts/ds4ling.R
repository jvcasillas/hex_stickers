...
p <- rnorm(10000) %>%
  as_tibble(.) %>%
  ggplot(., aes(x = value)) +
    geom_histogram(color = 'black', fill = 'grey70', binwidth = 0.5, size = 0.2) +
    annotate(geom = 'text', x = 10, y = 1100, label = course_name,
...
sticker(p, package = "SPAN589", url = url, p_family = "Aller_Rg",
        ...
        spotlight = T, l_y = 0.8, l_x = 0.7, l_alpha = 0.3, u_color = 'white',
